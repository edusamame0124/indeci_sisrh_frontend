...
import type { EmpleadoBancoRow } from '../../models/empleado-banco.model';
import type { PersonaEmpleado } from '../../models/persona-empleado.model';

@Component({
  selector: 'app-empleado-banco-list-page',
  standalone: true,
  imports: [
    RouterLink,
    MatCardModule,
  ...
  template: `
    <div class="page">
      ...
      <div class="actions">
        <a mat-button routerLink="/rrhh/cuentas-bancarias">Volver</a>
        @if (canEdit()) {
          <a
            mat-flat-button
            color="primary"
            [routerLink]="['/rrhh/cuentas-bancarias/personas', personaId(), 'nueva']"
          >
            Registrar cuenta
          </a>
        }
      ...
      @if (pageLoading()) {
        ...
        <mat-card class="page-card sisrh-elevated" role="alert">
          <mat-card-content>
            <p>No hay registro de empleado vinculado a esta persona. No se pueden gestionar cuentas.</p>
          </mat-card-content>
        </mat-card>
      } @else {
        <mat-card class="page-card sisrh-elevated">
          ...
          <mat-card-content>
            @if (tableLoading()) {
              <div class="loading" aria-busy="true">
                <mat-progress-spinner mode="indeterminate" diameter="40" aria-label="Cargando cuentas" />
              </div>
            } @else if (rows().length === 0) {
              <p class="empty" role="status">No hay cuentas activas registradas.</p>
            } @else {
              <div class="sisrh-table-scroll">
              <table mat-table [dataSource]="pagedRows()" class="tbl">
                <ng-container matColumnDef="banco">
                  <th mat-header-cell *matHeaderCellDef scope="col">Banco</th>
                  <td mat-cell *matCellDef="let row">{{ bankName(row.bankId) }}</td>
                </ng-container>
                <ng-container matColumnDef="numeroCuenta">
                  <th mat-header-cell *matHeaderCellDef scope="col">N° cuenta</th>
                  <td mat-cell *matCellDef="let row">{{ row.numeroCuenta }}</td>
                </ng-container>
                <ng-container matColumnDef="cci">
                  <th mat-header-cell *matHeaderCellDef scope="col">CCI</th>
                  <td mat-cell *matCellDef="let row">{{ row.cci || '—' }}</td>
                </ng-container>
                <ng-container matColumnDef="planilla">
                  <th mat-header-cell *matHeaderCellDef scope="col">Planilla</th>
                  <td mat-cell *matCellDef="let row">{{ row.esCuentaPlanilla === 1 ? 'Sí' : 'No' }}</td>
                </ng-container>
                <ng-container matColumnDef="acciones">
                  ...
                  <td mat-cell *matCellDef="let row">
                    <a
                      mat-icon-button
                      [routerLink]="[
                        '/rrhh/cuentas-bancarias/personas',
                        personaId(),
                        'editar',
                        row.id,
                      ]"
              ...
              <mat-paginator
                ...
                aria-label="Paginador de cuentas bancarias"
              />
            }
          </mat-card-content>
        </mat-card>
...
export class EmpleadoBancoListPageComponent implements OnInit {
  ...
  readonly columns = ['banco', 'numeroCuenta', 'cci', 'planilla', 'acciones'] as const;
  ...
  readonly bankLabels = signal<ReadonlyMap<number, string>>(new Map());
  ...
  readonly canEdit = computed(() => this.empleadoId() != null && this.empleadoId()! > 0);
  ...
  ngOnInit(): void {
    ...
    this.personaApi.obtenerPorId(pid).subscribe({
      next: (p) => {
        ...
        if (eid != null) this.loadBancos(eid);
      },
    ...
    this.catalogo.listarBancos().subscribe({
      next: (list) => {
        ...
        this.bankLabels.set(m);
      },
      error: () => undefined,
    });
  }

  bankName(id: number): string {
    return this.bankLabels().get(id) ?? `ID ${id}`;
  }
  ...
  private loadBancos(eid: number): void {
    this.tableLoading.set(true);
    this.bancoApi.listar(eid).subscribe({
      next: (list) => {
        this.rows.set(list);
