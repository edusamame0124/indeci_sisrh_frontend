...
export class GestionJefePageComponent implements OnInit {
  ...
  cargarSolicitudes(): void {
    ...
    this.service.listarSolicitudesColaboradores().subscribe({
      next: (resp) => {
        this.solicitudes.set(resp.data ?? []);
        this.cargando.set(false);
      },
      error: () => {
        this.error.set('No se pudo cargar las papeletas de los colaboradores.');
        this.cargando.set(false);
      },
    });
  }
  ...
  abrirAprobar(item: SolicitudRrhh): void {
    const ref = this.dialog.open(AprobarJefePapeletaDialogComponent, {
      width: '620px',
      maxWidth: '95vw',
      disableClose: true,
      data: item,
