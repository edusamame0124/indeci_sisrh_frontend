...
@Component({
  ...
  template: `
    <div class="page">
      ...
      <div class="actions">
        <a mat-button routerLink="/rrhh/puesto">Volver</a>
        @if (canRegistrarMovimiento()) {
          <a
            mat-flat-button
            color="primary"
            [routerLink]="['/rrhh/puesto/personas', personaId(), 'nueva']"
          >
      ...
      @if (pageLoading()) {
        ...
        <mat-card class="page-card sisrh-elevated">
          ...
          <mat-card-content>
            <p class="hint">
              Un nuevo registro cierra el puesto activo y abre el siguiente. Los identificadores de
              sede, oficina, nivel y jefe se muestran como códigos hasta disponer de catálogos.
            </p>
            ...
            @if (tableLoading()) {
              ...
              <p class="empty" role="status">No hay movimientos registrados.</p>
            } @else {
              ...
              <table mat-table [dataSource]="pagedRows()" class="tbl">
                ...
                <ng-container matColumnDef="nivelId">
                  <th mat-header-cell *matHeaderCellDef scope="col">Nivel ID</th>
                  <td mat-cell *matCellDef="let row">{{ row.nivelId ?? '—' }}</td>
                </ng-container>
                <ng-container matColumnDef="sedeId">
                  <th mat-header-cell *matHeaderCellDef scope="col">Sede ID</th>
                  <td mat-cell *matCellDef="let row">{{ row.sedeId ?? '—' }}</td>
                </ng-container>
                <ng-container matColumnDef="oficinaId">
                  <th mat-header-cell *matHeaderCellDef scope="col">Oficina ID</th>
                  <td mat-cell *matCellDef="let row">{{ row.oficinaId ?? '—' }}</td>
                </ng-container>
                <ng-container matColumnDef="jefeId">
                  <th mat-header-cell *matHeaderCellDef scope="col">Jefe ID</th>
                  <td mat-cell *matCellDef="let row">{{ row.jefeId ?? '—' }}</td>
                </ng-container>
